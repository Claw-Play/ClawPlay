...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(
      new URL("/login?error=discord_denied", request.nextUrl.origin)
    );
  }

  try {
    const accessToken = await exchangeDiscordCode(code, request.nextUrl.origin);
    ...
    return NextResponse.redirect(
      new URL("/login?error=discord_failed", request.nextUrl.origin)
    );
  }
}
